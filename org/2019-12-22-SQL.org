...
2. 少用*，限定字段选择，提高返回速度
3. 尽量少对主键使用in, not in, or，会导致全表扫描，效率低下，在主键列表可预测且少的情况下，可以改用各种动态SQL的技术生成单一SQL在union拼起来
4. like和null值判断，尽量在通过索引减少区间范围后再用
5. where表达式不要左侧运算
6. 不要用where 1=1，动态SQL判断有条件自动加where...和and
